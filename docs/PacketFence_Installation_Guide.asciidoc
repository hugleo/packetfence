...
We added the ability to specify filters directly in the portion of code that re-evaluates the VLAN or do a call to the API. These filters can
be defined in _Configuration -> Advanced Access Configuration -> Filter engines_.
...
EXAMPLE: create a security violaton if the SSID is OPEN and the owner is igmout
...
All the packetfence.* scopes are covered in the file radius_filters.conf, this is advanced configuration and you must know what you are doing.
...
For example, lets define a rule that return Access Accept when the connection is Ethernet-EAP and when there is no security event 
...
image::acme.com.2.png[scaledwidth="100%",alt="ACME Realm Configuration"]
...
This rule will do the following:
  ...
  if it matched then we will trigger a security event (with the trigger internal::888888) for the mac address of the device.
...
If your isolation and registration network are not routable on the network between PacketFence and this network then you can create a GRE Tunnel between the network equipment and PacketFence.

This setup has been tested in cluster mode but it should be easy to reproduce in standalone mode.

First you need to create the GRE configuration, let's assume that on the other side you have a Centos 7 machine.

 172.19.0.5 is the recheable ip address of the remote Centos 7 machine.
 192.168.0.5 is a VIP on the packetfence side.
 10.0.0.1 and 10.0.0.2 are the ip addresses used inside the tunnel.
 172.20.0.0/24 is the remote registration network.

On the PacketFence side edit the file /etc/sysconfig/metwork-script/ifcfg-gre1
 ...
 NM_CONTROLLED=no
 TYPE=GRE
 PEER_OUTER_IPADDR=172.19.0.5
 PEER_INNER_IPADDR=10.0.0.2/32
 MY_INNER_IPADDR=10.0.0.1/32
 MY_OUTER_IPADDR=192.168.1.5

On the other side edit the file /etc/sysconfig/metwork-script/ifcfg-gre1
 ...
 NM_CONTROLLED=no
 TYPE=GRE
 PEER_OUTER_IPADDR=192.168.1.5
 PEER_INNER_IPADDR=10.0.0.1/32
 MY_INNER_IPADDR=10.0.0.2/32
 MY_OUTER_IPADDR=172.19.0.5

And this file too edit the file /etc/sysconfig/metwork-script/route-gre1

 192.168.0.1 dev gre1

Next on both side do:

 ifup gre1

and try to ping 10.0.0.1 and 10.0.0.2 it should succeed.

Next step will be to add the VIP address on the PacketFence server, to do that edit the file /usr/local/pf/conf/cluster.conf and in the section CLUSTER interface ens192 (ens192 is your management interface)

 [CLUSTER interface ens192]
 ip=192.168.0.1
 alias=192.168.0.5

Last thing, you need to create a routed network in PacketFence by editing /usr/local/pf/conf/networks.conf

 [172.20.0.0]
 network=172.20.0.0
 dns=172.20.0.10
 gateway=172.20.0.1
 domain-name=vlan-registration.packeytfence.org
 pool_backend=mysql
 named=enabled
 netmask=255.255.255.0
...
Blocking malicious activities with security events 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Policy security events allow you to restrict client system access based on security events of certain policies. For example, if you do not allow P2P type traffic on your network, and you are running the appropriate software to detect it and trigger a security event for a given client, PacketFence will give that client a "blocked" page which can be customized to your wishes.
...
In order to make PacketFence react to the Snort alerts, you need to explicitly tell the software to do so. Otherwise, the alerts will be discarded. This is quite simple to accomplish. In fact, you need to create a security event and add the Snort alert SID in the trigger section of a security event.

PacketFence security events are configured in 'Configuration->Compliance->Security Events'

The example below will guide you to create a security event that will isolate device that have generated Peer-to-peer traffic and that are using Mac OSX or have a malware and are using Microsoft Windows

Security Event Definition
+++++++++++++++++++++++++

First you need to configure the security event definition

image::violation-definition.png[scaledwidth="100%",alt="Security event definition"]

Where :
[options="compact"]
* `Enabled` is whether or not the security event is currently activated (should be `ON`).
* `Identifier` is the security event ID. Any integer except 1200000-120099 which is reserved for required administration security events.
* `Description` is the user friendly description of the security event.
* `Actions` is the list of actions to be executed when this security event is raised.
** `Unregister node` will unregister the node.
** `Send email to owner` will email the security event details to the owner of the device. Will only work if the person has it's email field populated.
** `Send email to admin` will email the security event details to the address specified in `[alerting].emailaddr`, using `[alerting].smtpserver`. Multiple `emailaddr` can be separated by comma.
** `Reevaluate access` will place the device in the destination VLAN configured in the security event. It opens a security event and leaves it open. If it is not there, the security event is opened and then automatically closed.
** `Log message` will log the security event in the log file security_event.log.
** `External command` will execute a command on the operating system when this security event is raised.
** `Close a security event` will close an existing security event for this device when this one is raised.
...
* `Priority` defines the order onto which security events should be handled should there be more than one for a device.
* `Whitelisted Roles` is the list of roles that are not affected by this security event.

Triggers
++++++++

Next, in the `Triggers` tab, you need to define the triggers that will raise the security event. In the case of this example it will be these two cases :
...
image::violation-triggers.png[scaledwidth="100%",alt="Security event triggers"]
...
image::violation-remediation.png[scaledwidth="100%",alt="Security event remediation"]

Where :
[options="compact"]
* `Auto Enable` is whether or not the user can release the security event himself after acknowledging the message on the captive portal.
* `Max Enables` is the amount of time a user can use the `Auto Enable` functionality. After this amount of times, he will not be able to release the security event and it will have to be manually release by an administrator using the PacketFence administration interface.
* `Grace` is Amount of time before the security event can reoccur. This is useful to allow hosts time (in the example 2 minutes) to download tools to fix their issue, or shutoff their peer-to-peer application.
* `Dynamic Window` will only work for accounting security events. The security event will be opened according to the time you set in the accounting security event (ie. You have an accounting security event for 10GB/month. If you bust the bandwidth after 3 days, the security event will open and the release date will be set for the last day of the current month).
* `Window` is the amount of time before a security event will be closed automatically. Instead of allowing people to reactivate the network, you may want to open a security event for a defined amount of time instead.
* `Delay by` is the delay before triggering the security event.
* `Template` is the HTML template the host will be redirected to while in security event. You can create new templates from the Connection Profiles configuration section.
* `Button text` is the text of the button that is used when the user is releasing the security event directly from the captive portal.
